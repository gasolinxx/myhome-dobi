...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <h4 class="">Upload Proof of Deliver</h4>
                        <p class="text-muted font-13 mb-4">
                            Please Fill the Form Below
                        </p>
                        <form action="{{ route('delivery-proof.deliver', $delivery->id) }}" method="POST"
                            ...
                            <div class="row justify-content-center align-items-center g-2">
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" name="deliver_id" class="form-control"
                                            value="{{ $delivery->deliveryDriver->name }}" readonly>
                                    </div>
                                </div>

                                <div class="form-group col-md-12">
                                    <label for="proof-file-{{ $order->id }}">Upload Proof Delivery</label>
                                    <input type="file" name="proof_deliver" class="form-control"
                                        id="proof-file-{{ $order->id }}" accept="image/jpeg,image/png" required>
                                </div>
                            </div><!-- end row-->
                            <div class="text-center mt-2">
                                <button type="button" onclick="history.back()" class="btn btn-light mr-3">Back</button>
                                <button id="confirm-button-{{ $order->id }}" class="btn btn-info" disabled>Confirm Proof
                                    of Deliver</button>
                            </div>
                        </form>
    ...
    <script>
        // Enable the Confirm button if a file is selected
     ...
     //--===== USER ERROR PROTECTION (S2): Tactic 2-Use Confirmation Prompts Before Finalizing Upload =====-- 

        // Add confirmation prompt before submitting the form
        document.getElementById('confirm-button-{{ $order->id }}').addEventListener('click', function (event) {
            const confirmation = confirm('Are you sure you want to confirm this delivery proof? Once submitted, changes cannot be made.');
